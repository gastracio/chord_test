...
def create_user(keyboard, identifier, password):
...
def creating_main_admin(keyboard, identifier, password):
    logging.info("Переход в настройки гл. администратора")
    keyboard.press("ENTER")
    logging.info("Создание пользователя")
    create_user(keyboard, identifier, password)
...
def test_chord_main_admin(id_number, keyboard, config, clear_db, log_test_borders):
    ...
    main_admin_password = "12345678"
    creating_main_admin(keyboard, config["identifiers"][id_number], main_admin_password)
    ...
    check_correctness_of_authentication()
    system_reboot()
    ...
    logging.info("Корректная аутентификация")
